...
describe('PegasysV3Factory', () => {
  const [wallet, other] = getWallets()

  let factory: PegasysV3Factory
  let poolBytecode: string
  const fixture = async () => {
    return (await deployContract('PegasysV3Factory')) as PegasysV3Factory
  }

  before('load pool bytecode', async () => {
    poolBytecode = (await loadArtifact('PegasysV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractArtifact = await loadArtifact('PegasysV3Pool')
