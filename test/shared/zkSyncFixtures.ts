...
export const poolFixture = async function (): Promise<PoolFixture> {
  ...
  const poolContractArtifact = await loadArtifact('MockTimeUniswapV3Pool');
  ...
  return {
    ...
    createPool: async (fee, tickSpacing, firstToken = token0, secondToken = token1) => {
      const mockTimePool = (await deployContract('MockTimeUniswapV3PoolDeployer')) as MockTimeUniswapV3PoolDeployer
      const tx = await mockTimePool.deploy(
        factory.address,
        firstToken.address,
        secondToken.address,
        fee,
      ...
      return new Contract(receipt.events?.[2].args?.pool as string, poolContractArtifact.abi, (await getWallets())[0])
    },
  }
}
