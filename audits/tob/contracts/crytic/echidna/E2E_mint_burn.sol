...
import '../../../../../contracts/PegasysV3Pool.sol';
...
contract E2E_mint_burn {
    ...
    PegasysMinter minter;
    PegasysSwapper swapper;
    ...
    constructor() public {
        ...
        pegasys = new SetupPegasys(token0, token1);

        minter = new PegasysMinter(token0, token1);
        swapper = new PegasysSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        PegasysMinter.MinterStats memory bfre,
        PegasysMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        pegasys.createPool(poolParams.fee, poolParams.startPrice);
        pool = pegasys.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (PegasysMinter.MinterStats memory bfre, PegasysMinter.MinterStats memory aftr) =
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            PegasysMinter.MinterStats memory bfre_burn,
            PegasysMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
